      ...
      {step === 2 && (
        <>
          <main className="flex-1 px-4 sm:px-6 lg:px-10 pb-8 sm:pb-10 pt-4 overflow-y-auto">
            {/* Header Form */}
            <Card className="mb-8 flex w-full flex-row items-center gap-4 rounded-3xl border border-slate-100 p-4 sm:w-fit sm:gap-5 sm:p-6 sm:pr-16">
              ...
              {visibleQuestions.map((q, idx) => {
                ...
                return (
                  <Card key={q.id} className="rounded-3xl border border-slate-100 p-6">
                    <h4 className="mb-4 break-words text-sm font-bold text-slate-800">{idx + 1}. {q.question_text} {isRequiredQuestion(q) && <span className="text-red-500">*</span>}</h4>
