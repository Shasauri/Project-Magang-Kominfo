...
export default function AdminLayout({
  ...
  // State untuk mengontrol pop-up logout & data user
  const [isLogoutModalOpen, setIsLogoutModalOpen] = useState(false);
  const [userName, setUserName] = useState("Admin");
  const [isSidebarOpen, setIsSidebarOpen] = useState(true);

  // Ambil nama user untuk ditampilkan di Pop-Up Keluar
  useEffect(() => {
    const fetchUser = async () => {
      ...
      try {
        ...
        setUserName(data.name || "Nabila Aulia");
      } catch (error) {
  ...
  // Helper untuk menentukan judul dinamis di Topbar
  const getPageTitle = () => {
    if (pathname === "/admin") return "Dashboard";
    if (pathname.includes("/admin/pengguna")) return "Daftar Pengguna";
    if (pathname.includes("/admin/profil")) return "Profil Akun";
    if (pathname.includes("/admin/reports")) return "Laporan Pengisian Form";
            ...
            <div className="flex flex-col gap-1">
              <Link 
                ...
                className={`flex items-center gap-3 px-4 py-3 rounded-xl text-sm font-semibold transition-all ${
                  pathname === "/admin" || pathname.includes("/admin/reports")
